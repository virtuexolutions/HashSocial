...
import { View, StyleSheet, I18nManager, NativeTouchEvent, TouchableOpacity, Image, Text, Dimensions } from 'react-native';
import { WP, pointOnCircle, calculateRadius, calculateAngleBetweenTwoPoints } from 'react-native-rotating-menu/src/services';
import { PanGestureHandler } from 'react-native-gesture-handler';
import { StylesheetInterface, PropsI, pointI } from 'react-native-rotating-menu/src/services/interfaces';
...
const RoundMenu = ({
    ...
    const findCenterpoints = (x:any, y:any )=>{
        ...
        const xRadius = (calculateDistance(xPoint2[0] , xPoint2[1], xPoint1[0], xPoint1[1]))/2

        // const yPoint1 =( ) && [x,y]
        // const yPoint2 =  ( ) && [x,y]
        const yRadius = ( calculateDistance(yPoint1[0] , yPoint1[1], yPoint2[0], yPoint2[1]))/2
        setPointsDone(true)
        return{xCenter:xcenter, yCenter: ycenter, xRadius:xRadius, yRadius:yRadius}
        // return(
        //     <Svg>
    ...
    return (
        <View >
            <PanGestureHandler
                ...
                <View 
                >
                    {centerContent && <Animatable.View ref={innerContainerRef}
                    style={{
                        transform:  [{  rotate: `${rotationAngle}deg`,  }],
                        left: alignment == 'left' ? center.x - centerContentSize / 1.7 :center.x - centerContentSize / 2.3 ,
                ...
                <Animatable.View ref={outerContainerRef}                  
                 style={styles({ size, backgroundColor, }).container}>
                    {content.map((el, i) => {
                    const [x, y] = pointOnCircle({
                        radius,
                        angle: divisionAngle * (i + 1) + offsetAngle + rotationAngle + 90,
                        cx: center.x,
                        cy: center.y
                    });
                    ...
                    const elContainerSize = alignment == 'left' ?  x /((Math.round(x)>=minx&&Math.round(x)<maxx)&&(Math.round(y)>=miny&&Math.round(y)<maxy) ? 3 : 4 ) : (Dimensions.get('window').width - x)/((Math.round(x)>=minx&&Math.round(x)<maxx)&&(Math.round(y)>=miny&&Math.round(y)<maxy) ? 3 : 4 );
                    if((Math.round(x)>=minx&&Math.round(x)<maxx)&&(Math.round(y)>=miny&&Math.round(y)<maxy) ){
                    ...
                    return (<>
                        ...
                        <Animatable.View
                          animation={(elContainerSize == x /3 || elContainerSize ==(Dimensions.get('window').width - x) /3)  && (el?.bubble ? 'pulse' : "swing")}
                          easing="ease-in"
                          iterationCount="infinite"
                            style={[
                                styles({ elContainerSize, elContainerCoOrdinates: { x, y } }).elContainer,
                                {
                                    ...
                                    borderRadius: el?.bubble ? elContainerSize/2 : elContainerSize/5 ,
                                    borderWidth: elContainerSize == x /3 || elContainerSize == (Dimensions.get('window').width - x) /3 ?3  : 1,
                                    overflow:'hidden',
                            ...
                            if(el?.bubble &&  el?.private){
                                el?.item?.profile_id == profileData?.id ? setclicked(true) : setIsVisible(true)
                                setText('bubble')
                            ...
                            >   
                                ...
                                position:'absolute',
                                 ...
                                 alignItems:"center",
                                 justifyContent:'center'}}>
                              <Icon as={AntDesign} name="lock" size={elContainerSize/6} color={"black"} />
                            </View>}
                                 <Image source={el.source} style={{width:'100%', height:'100%'}} />
